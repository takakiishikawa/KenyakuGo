...
export async function GET() {
  ...
  const [last7Res, prev7Res, thisMonthRes, recentRes, settingsRes, allMonthsRes] =
    await Promise.all([
      db
        .from("transactions")
  ...
  const errors = [last7Res.error, prev7Res.error, thisMonthRes.error, recentRes.error].filter(Boolean);
  if (errors.length > 0) {
  ...
  const damMonths = calcDamMonths({ txs: allMonthTxs, targetMonthly, fixedCosts, now });
