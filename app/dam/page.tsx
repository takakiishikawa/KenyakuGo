...
export default function DamPage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-semibold mb-10" style={{ color: "var(--kg-text)" }}>貯蓄ダム</h1>

      <div className="grid grid-cols-2 gap-5 mb-6">
        <div className="kg-card-static p-6 animate-fade-up">
          <p className="text-xs font-medium uppercase tracking-widest mb-1" style={{ color: "var(--kg-text-muted)" }}>今月の貯水状況</p>
          <DamVisual level={data?.achievementRate ?? 0} />
        </div>

        <div className="flex flex-col gap-5">
          {[
            { label: "今月の貯水量", value: data ? formatVND(data.currentBalance) : "—", color: data && data.currentBalance >= 0 ? "var(--kg-success)" : "var(--kg-danger)", delay: 80 },
            { label: "達成率", value: data ? `${data.achievementRate}%` : "—", color: "var(--kg-accent)", delay: 160 },
            { label: "累計ダム残高", value: data ? formatVND(data.cumulativeBalance) : "—", color: "var(--kg-text)", delay: 240 },
          ].map((card) => (
            <div key={card.label} className="kg-card p-7 animate-fade-up flex-1" style={{ animationDelay: `${card.delay}ms`, animationFillMode: "both" }}>
              <p className="text-xs font-medium uppercase tracking-widest mb-3" style={{ color: "var(--kg-text-muted)" }}>{card.label}</p>
              <p className="font-num text-3xl font-semibold" style={{ color: card.color }}>{card.value}</p>
      ...
      <div className="kg-card-static p-7 animate-fade-up" style={{ animationDelay: "280ms" }}>
        ...
        <div className="border-l-2 pl-4" style={{ borderColor: "var(--kg-accent)" }}>
          {commentLoading ? (
            ...
            <p className="text-sm" style={{ color: "var(--kg-text-muted)" }}>設定画面で想定月支出を設定すると提案が表示されます</p>
          )}
        </div>
      </div>
    </div>
