...
import { TrendingDown, TrendingUp, Sparkles, RefreshCw, ChevronRight } from "lucide-react";
import { toast } from "sonner";
import { formatVND, formatDate } from "@/lib/format";
import { getCategoryColors } from "@/lib/category-colors";
import {
  Button, Card, Badge, PageHeader, Skeleton,
      ...
      <DialogContent className="max-w-md p-0 overflow-hidden flex flex-col gap-0">
        ...
        <div className="overflow-y-auto" style={{ maxHeight: 400 }}>
          {txs === null ? (
            ...
            <div>
              {txs.map((tx) => (
                <div key={tx.id} className="flex items-center justify-between px-6 py-3.5 border-b last:border-0 hover:bg-muted/30 transition-colors">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium truncate text-foreground">{tx.store}</p>
                    <p className="text-xs mt-0.5 text-muted-foreground">{formatDate(tx.date)}</p>
                  </div>
                  <p className="font-num text-sm font-semibold ml-4 shrink-0 text-foreground">{formatVND(tx.amount)}</p>
                </div>
              ))}
            </div>
          )}
    ...
    <Dialog open={true} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-w-lg p-0 overflow-hidden flex flex-col gap-0">
        ...
        <div className="flex items-start justify-between px-6 pt-6 pb-5 border-b pr-14">
          <div>
            <h2 className="text-base font-semibold text-foreground">直近7日間 vs 前の7日間</h2>
            <p className="text-sm text-muted-foreground mt-0.5">カテゴリ別の支出変化</p>
          </div>
        ...
        <div className="overflow-y-auto" style={{ maxHeight: 320 }}>
          {rows.map(({ cat, current, prev, diff }) => (
            <div key={cat} className="grid grid-cols-[1fr_auto_auto_auto] gap-x-4 items-center px-6 py-3 border-b last:border-0 hover:bg-muted/30 transition-colors">
...
export default function Dashboard() {
  ...
  const donutConfig = useMemo<ChartConfig>(() => {
    ...
    (data?.categoryBreakdown ?? []).forEach((item, i) => {
      cfg[item.name] = { label: item.name, color: DONUT_COLORS[i % DONUT_COLORS.length] };
    });
    return cfg;
    ...
    <div>
      ...
      <div className="mt-8 grid grid-cols-3 gap-5 mb-8">
        ...
        <Card className="p-7 animate-fade-up" style={{ animationDelay: "160ms", animationFillMode: "both" }}>
          <p className="text-xs font-medium uppercase tracking-widest mb-3 text-muted-foreground">累計ダム残高</p>
          ...
          {data && (
            <p className="mt-2 text-sm font-medium flex items-center gap-1"
              style={{ color: data.cumulativeBalance >= 0 ? "var(--kg-success)" : "var(--kg-danger)" }}>
              {data.cumulativeBalance >= 0 ? <TrendingDown size={14} /> : <TrendingUp size={14} />}
        ...
        <Card className="p-7 animate-fade-up" style={{ animationDelay: "200ms" }}>
          ...
          {data?.categoryBreakdown?.length ? (
            <div className="flex flex-col items-center gap-4">
              <ChartContainer config={donutConfig} className="aspect-auto h-[220px] w-full">
                <PieChart>
